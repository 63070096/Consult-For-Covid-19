...
int question_page() {
	...
	for(int i = 0 ;;) {
		...
		pos_xy(12,8);
		color(7);
		printf("\n\tQ [%d] | %s\n", covid_point, question[num_question]);	
        pos_xy(12,12);
        color(set[0]);
        printf("> True");
...
int result_page() {
	...
	for(int i = 0 ;;) {
        ...
        color(7);
        if(covid_point == 0) {
        	printf("> Result : %s Safeee !!!", info.firstname);
		} else {
...
int result_page() {
    ...
    for(int i = 0 ;;) {
    	...
    	pos_xy(27,10);
        color(set[0]);
        printf("Hello %s %s, your test has been checked!", info.firstname, info.lastname );

    	pos_xy(25,12);
		if(covid_point >= 150) {
			color(12);
			printf("> Result : High Risk to infected COVID-19!");
		} else if(covid_point >= 100 && covid_point <= 150) {
			color(4);
			printf("> Result : Risk to infected (should see a doctor to test )!");
		} else if(covid_point >= 50 && covid_point <= 100) {
			color(14);
			printf("> Result : You should Quanrantine 14 day and wait to see");
		} else if(covid_point >= 20 && covid_point <= 50) {
			color(2);
			printf("> Result : Low risk but have chance should protect yourself");
		} else if(covid_point < 20) {
			color(10);
			printf("> Result : Very low risk but protect yourself first");
		}
